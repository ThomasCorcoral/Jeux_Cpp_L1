...
const int DIMW = 500;                   // Taille de la fen�tre
const int NMAX = 200;
const float dt = 0.005f;
const float G = 9.81f;                   // Gravit� sur la Terre
const float Gl = 1.61f;                  // Gravit� sur la lune
...
struct Personnage
{
    Image Im;       // Image associ�e au personnage
    Vec2 coord;     // Un personnage est associ� � un point qui est donc un Vec2
};
...
struct robot
{
    ...
    Vec2 coord;     // Coordonn�s du robot
    Vec2 v;         // Vitesse
...
struct mini_jeu1
{
	...
	int score;                  // nombre de particules attrap�s
};
...
struct DirectionMj3
{
    ...
    int direction;          // Valeur � 0 ou 1  en fonction de la droite ou la gauche
};

struct aste     // Structure d'un ast�roide
{
    Particle part[NMAX];        // tableau de particules
	int np;                     // nombre de particules
	...
	Vec2 centre;                // centre de l'ast�roide
};
...
struct mini_jeu3
{
    ...
    aste ast[NMAX];         // ast�roides
    int na;                 // nombre ast�roides

	float dt;
};

struct arme     // Structure d�finissant une arme
{
    float delai;        // Temps de rechargement
    int degat;          // D�gat inflig�s par tir
    int conso_part;     // Consommation de particules par tir
    float travel_time;  // Vitesse de la balle
    Image Im_arme;      // Image associ�e � l'arme
};

struct mini_jeu4
{
    robot Robot;              // Robot � battre
...
struct World
{
    mini_jeu1 mj1;              // Chaque mini-jeu � une structure associ�e
	...
	Image Im_pres[NMAX];        // Pr�sentation du projet
	...
	int TimeStart;              // temps d�but mini jeu
};
...
void updateParticle(mini_jeu3 &mj3, float dt)   // Mise � jour des particules du mini-jeu n�3
{
	int i, j;
	for (j = 0; j < mj3.na ; j++)   // On parcourt la liste des ast�roides
	{
        for (i = 0; i < mj3.ast[j].np; ++i)     // On parcourt la liste des particules de chaque ast�roides
        {
...
void init(World &w)
{
    ...
    w.Im_pres[0] = image("/data/projet/Pres1.png");
    w.Im_pres[1] = image("/data/projet/Pres2.png");
    w.Im_fond[1] = image("/data/projet/fond0.jpg");
    w.Im_fond[2] = image("/data/projet/fond1.png");
    w.Im_fond[3] = image("/data/projet/fond2.jpg");
    w.Im_fond[4] = image("/data/projet/fond3.png");
    w.Im_fond[5] = image("/data/projet/fond4.jpg");
    w.Im_fond[6] = image("/data/projet/fond5.png");
    w.Im_fond[7] = image("/data/projet/fond6.png");
    w.Im_fond[8] = image("/data/projet/fond7.png");
    w.mj2.Perso.Im = image("/data/projet/perso.png");
    w.mj3.direct.dir[0] = image("/data/projet/astronaute_gauche.png");
    w.mj3.direct.dir[1] = image("/data/projet/astronaute_droite.png");
    w.Im_transition[0] = image("/data/projet/transition0.png");
    w.Im_transition[1] = image("/data/projet/transition1.png");
    w.mj4.Robot.Im = image("/data/projet/robot.png");
    w.mj4.Viseur.Im = image("/data/projet/viseur.png");
}
...
void init_Perso(World &w)
{
    ...
    // Initialisation des coordonn�s du personnage pour le 2eme mini-jeu
...
void init_armes(World &w)
{
    ...
    w.mj4.Arme[0].Im_arme = image("/data/projet/lanceroquette.png");
    ...
    w.mj4.Arme[1].Im_arme = image("/data/projet/sniper.jpg");
    ...
    w.mj4.Arme[2].Im_arme = image("/data/projet/deagle.jpg");
    ...
    w.mj4.Arme[3].Im_arme = image("/data/projet/sulfateuse.jpg");
    ...
    w.mj4.Arme[4].Im_arme = image("/data/projet/pistoletsil.png");
}
...
void Presentation(World &w)
{
    ...
    if(elapsedTime() < 20)
    {
        image_draw(w.Im_pres[0] , 0 , 0 , DIMW-1 , DIMW-1 );         // affiche la premi�re image de pr�sentations.
    }
...
void transition2(World &w)
{
    ...
    // D�limitation des zones pour le choix des armes
...
void fin(World &w)
{
    ...
    if(w.Step == 7)
    {
        ...
        // On v�rifie si le robot a encore de la vie ou non.
...
void drawSeau(Vec2 v1, Vec2 v2, Vec2 v3, Vec2 v4)
{
    ...
    triangleFill(v1.x , v1.y , v2.x , v2.y , v4.x , v4.y);  // On aurait �galement pu faire un rectangle mais je voulais essayer cette technique.
    color(0,0,0);
...
Particle putPartMj1()
{
    ...
    P.p.y = DIMW-4;
    P.v.x = 0;                       // Pas de vitesse au d�but
    P.v.y = 0;
    P.f.x = frand(-4, 4);            //Force lat�rale entre -4 et 4
    P.f.y = -G;                      // Effet de la gravit� terrestre
    P.m = 1;                         // masse � 1 par d�faut
    P.etat = true;                   // Toutes les particules sont activ�s de base
    return P;
}
...
void updateMj1(World &w)
{
    ...
    {
        if(w.mj1.Seau[2].x-1 > 0)                      // Bloque le seau � la bordure gauche
        {
    ...
    {
        if(w.mj1.Seau[3].x+1 < DIMW)                   // Bloque le seau � la bordure droite
        {
            w.mj1.Seau[0].x = w.mj1.Seau[0].x + 1;
            w.mj1.Seau[1].x = w.mj1.Seau[1].x + 1;
            w.mj1.Seau[2].x = w.mj1.Seau[2].x + 1;
    ...
    if(w.mj1.np < time && w.mj1.np < 100) // fait apparaitre une particule par seconde avec un maximum de 50 particules
    {
        w.mj1.np++;
    ...
    for(i=0; i < w.mj1.np ; i++)
    {
        ...
        // v�rification que la particule est toujours active
    ...
    // Si le temps est fini (101 secondes) on passe au mini-jeu suivant
...
void updateMj2(World &w)
{
    ...
    // On bloque le personnage dans la fus�e
    ...
    // V�rifications des contacts entre le joueur et les particules
    ...
    // Si le joueur � atteint la manette
...
int addPart(aste &as, float x, float y, float m=1) // Initialisation basique d'une particule d'ast�ro�de
{
    int i = as.np;
...
void addAsteroide(mini_jeu3 &mj3)   // Cr�ation d'un ast�ro�de
{
    int aleatoire;
    ...
    mj3.ast[mj3.na-1] = as;     // On ajoute l'ast�ro�de au tableau d'ast�roide de la structure du mini jeu.
}

void collisionLune(mini_jeu3 &mj3) // G�re les collisions de l'ast�ro�de avec les bordures sur la lune.
{
	int i, j, k;
	for (j = 0; j < mj3.na ; j++)
    {
...
void collisionAstronaute(mini_jeu3 &mj3)
{
	...
	for (j = 0; j < mj3.na ; j++)
    {
        for (i = 0; i < mj3.ast[j].np; ++i)
        {
            ...
            // V�rification que chaque particule de chaque ast�ro�de n'est pas en contact avec l'astronaute.
...
void collisionAsteroides(mini_jeu3 &mj3)
{
    ...
    if(mj3.na == 2)
    {
        // v�rification de la distance entre les deux centres
        ...
        if ( y < 50)
        {
            ...
            {
                // Si les ast�ro�des sont en contact on inverse les vitesses
...
{
    int i;
    // Mise � jour du centre pour tous les ast�ro�des
    for(i=0; i<mj3.na ; i++)
    {
...
void updateMj3(World &w)
{
    ...
    if(isKeyPressed(SDLK_LEFT))
    {
        if(w.mj3.Perso.x-10 > 0)                      // Bloque l'astronaute � la bordure gauche
        {
            w.mj3.direct.direction = 0;
    ...
    if(isKeyPressed(SDLK_RIGHT))
    {
        if(w.mj3.Perso.x+10 < DIMW-129)                   // Bloque l'astronaute � la bordure droite
        {
    ...
    updateSpring(w.mj3);
    int time = elapsedTime() - w.TimeStart;
    if(time <= 60 && w.mj3.na < time/3+1 && w.mj3.na < 2) // ajout d'ast�ro�de
    {
        w.mj3.na++;
        addAsteroide(w.mj3);
    }

    // Cas d'arr�t du mini-jeu
...
void AddBalle(mini_jeu4 &mj4)
{
    ...
    int nb = mj4.nb;
    float tt = mj4.Arme[mj4.choix_arme].travel_time;   // travel_time / Beaucoup plus court � utiliser
    ...
    // Calculs math�matiques pour trouver la vitesse en x.
    ...
    // la variable oppos� ne pouvant �tre n�gative pour la suite des calculs
    // on reporte le signe moins dans une autre varible en y assignant la valeur -1
    // Puis on multipliera plus tard la vitesse pas cette variable pour avoir une vitesse en x n�gative.
    ...
    float angle_max = 26.7515;      // Angle maximum constat� sur une fen�tre avec DIMW = 500.

    mj4.Balle[nb].v.x = tt * (angle/(angle_max*(adjacent/(DIMW/2-29))) * droite_gauche);

    // Initialisation des force � 0 car elles sont n�glig�s dans ce mini jeu
...
void mouv_robot(robot &Robot)
{
    ...
    else
    {
        ...
        {
            int r1;
            r1= frand(-4,4);        // D�placements al�atoires du robot
            Robot.v.x += r1;
        }
    }
...
void updateMj4(World &w)
{
    ...
    if(nb_tir > 0)
    {
        if(w.mj4.Robot.vie > 0)
        {
            ...
            mousePos(x,y);
            // On bloque les coordonn�s du viseur
            if(x <= 29)
            {
            ...
            // Si l'utilisateur clique et que le d�lai de rechargement est d�pass�
            ...
            for(i=0; i < w.mj4.nb ; i++)
            {
                // Si la balle est active, on la met � jour et on v�rifie les cas de collision avec le robot
                if(w.mj4.Balle[i].etat)
                {
        ...
        {
            // Si plus de tirs, on passe � l'�tape suivante
            w.Step++;
            w.TimeStart = elapsedTime();
        }
    }
